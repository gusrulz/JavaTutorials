...
public class Framework extends Canvas {
	 ...
	 * This method is intended to set the variables and objects for this class, variables and objects for the actualy game can be set in Game.java.
	 */
	...
	private void GameLoop() {
		...
		// These two variables are used in VISUALIZING state of the game. We use them to wait some time so that we get correct frame/window resolution.
		long visualizingTime = 0, lastVisualizingTime = System.nanoTime();
		
		// These variables are used for calculating the time that defines for how long we should put thread to sleep to meet the GAME_FPS.
		...
		while(true) {
			...
			switch(gameState) {
				...
				case STARTING:
					...
					// When all thing that are called above are finished, we change game status to main menu.
			...
			// If the time is less than 10 milliseconds, then we will put thread to sleep for 10 milliseconds so that some other thread can do some work.
			if(timeLeft < 10) {
