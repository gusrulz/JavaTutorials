...
public class Animation {
	...
	// Amount of time between frames in milliseconds. (How much time in milliseconds will one frame be shown before showing the next frame?)
	...
	/** State of animation. Is it stil active or is it finished? We need this so that we can check and delete animation when it is finished. */
	...
	private void Update() {
		...
		if(timeForNextFrame <= System.currentTimeMillis()) {
			...
			// If the animation has reached the end, we restart it by setting current frame to zero. If the animation isn't looped then we set the animation as inactive
